...
func (self *InstallModule) Init(env *Env, router *httprouter.Router) {
	self.super("install", env, router)
	self.router.GET("/install", self.ViewInstallIndex)
}
...
func (self *InstallModule) ViewInstallIndex(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	self.RenderResponse(w,"index.html", context)
}
